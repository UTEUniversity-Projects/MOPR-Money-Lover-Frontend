...
import com.moneylover.ui.main.app.transactionHistory.viewReport.netIncome.adapter.NetIncomeAdapter;
...
public class NetIncomeDetailFragment extends BaseFragment<FragmentNetIncomeDetailBinding, NetIncomeDetailViewModel> {
    ...
    public void setupNetIncome() {
        List<NetIncome> netIncomes = new ArrayList<>();
        netIncomes.add(new NetIncome("01/04-06/04", 4_800_000, 200_000));
        netIncomes.add(new NetIncome("07/04-13/04", 2_000_000, 3_000_000));
        netIncomes.add(new NetIncome("14/04-20/04", 1_000_000, 1_500_000));
        netIncomes.add(new NetIncome("21/04-27/04", 3_000_000, 500_000));
        netIncomes.add(new NetIncome("28/04-04/05", 2_000_000, 1_000_000));

        NetIncomeAdapter adapter = new NetIncomeAdapter(getContext(), netIncomes, new OnItemClickListener() {
            @Override
            public void onItemClick(int position) {
                NavigationUtils.navigateToActivity((AppCompatActivity) getActivity(), NetIncomeTransactionActivity.class, R.anim.slide_in_right, R.anim.slide_out_left);
            }
        ...
        binding.rcvNetIncome.setLayoutManager(new LinearLayoutManager(getContext(), LinearLayoutManager.VERTICAL, false));
        binding.rcvNetIncome.setAdapter(adapter);
    }
}